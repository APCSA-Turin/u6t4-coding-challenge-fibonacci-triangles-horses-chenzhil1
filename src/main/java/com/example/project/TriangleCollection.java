  ...
  public void shiftTriangles(int increment) {
    ...
    for(int i = 0; i < collection.length; i ++) {
      ...
      for(int j = 0; j < points.length; j ++) {
        int previousX = points[j].getX();
        int previousY = points[j].getY();
        int newX = previousX + increment;
        int newY = previousY + increment;
        points[j].setPoint(newX, newY);
      }
    }
  }
